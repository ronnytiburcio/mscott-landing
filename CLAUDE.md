...
- `src/components/world/StaticWorld.jsx` — owns the 6 scene sections' layout/backgrounds. Scene copy components in `src/components/scenes/` are **pure content** (no scroll logic, no backgrounds) so a future scroll-world build can portal them into the scrub engine unchanged.
